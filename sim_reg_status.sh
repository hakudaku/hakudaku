...
# - - Loop through each file - looking for the sim_reg_status_Item used (field X)
# - - - Push up MyxID (field 1) with the sim_reg_status_Item (field X) into Graphite
...
# Main
  ...
  do
    ...
    # read nm
    Bad_sim_reg_status_Item_Entries=0; Good_sim_reg_status_Item_Entries=0    # UPDATE_HERE: REPLACE Your Variables HERE - Example: sim_reg_status_Item
    ...
    do
      ...
      Row=`echo "${Line}" | awk -F"," '{print $1","$57","$17}'` # UPDATE_HERE: Update $57 to whatever field you are looking for
      ...
      if [ "${MyxID} " = " " ] ; then
        ...
        echo "echo \"Housekeeping.Ingestion.sim_reg_status_Item.Failed 100 ${Data_Epoc}\" | nc ${GraphiteServer} ${GraphitePort}" >> ${ScriptDir}/${ScriptNameGraphiteUpload}
        break # Exit this loop and skip to the next file
      fi

      sim_reg_status_Item=""; sim_reg_status_Item=`echo ${Row} | awk -F"," '{print $2}'`
      if [ "${sim_reg_status_Item} " = " " ] ; then
        # echo " "
        # echo "sim_reg_status_Item is Missing - aka NULL - aka ,, "
        echo "echo \"LMS.Device.Issue.${MyxID}.sim_reg_status_Item.Null 100 ${Data_Epoc}\" | nc ${GraphiteServer} ${GraphitePort}" >> ${ScriptDir}/${ScriptNameGraphiteUpload}
        Bad_sim_reg_status_Item_Entries=`expr ${Bad_sim_reg_status_Item_Entries} + 1`
        Echo "!"
        Current_Row=`expr ${Current_Row} + 1`
        continue
      fi

      sim_reg_status_Item=""; sim_reg_status_Item=`echo ${Row} | awk -F"," '{print $2}' | tr -d '"'`
      if [ "${sim_reg_status_Item} " = " " ] ; then
        # We have a malformed (EMPTY) sim_reg_status_Item entry - it should not be blank for this data type - aka ""
        echo "echo \"LMS.Device.Issue.${MyxID}.sim_reg_status_Item.Malformed 100 ${Data_Epoc}\" | nc ${GraphiteServer} ${GraphitePort}" >> ${ScriptDir}/${ScriptNameGraphiteUpload}
        # echo " "
        # echo "sim_reg_status_Item is Blank - aka EMPTY - aka doublequote thing "
        Bad_sim_reg_status_Item_Entries=`expr ${Bad_sim_reg_status_Item_Entries} + 1`
        Echo "x"
        Current_Row=`expr ${Current_Row} + 1`
        continue
        ...
        sim_reg_status_Item_Converted=""
        sim_reg_status_Item_Converted=`echo "${sim_reg_status_Item}" | tr ' ' '_'`
        sim_reg_status_Item_Converted=`echo "${sim_reg_status_Item_Converted}" | tr ',' '_'`
        sim_reg_status_Item=${sim_reg_status_Item_Converted}
        # We are good - let's update the details
        Echo "."
        echo "echo \"LMS.Device.MyxID.${MyxID}.sim_reg_status_Item.${sim_reg_status_Item} 100 ${Data_Epoc}\" | nc ${GraphiteServer} ${GraphitePort}" >> ${ScriptDir}/${ScriptNameGraphiteUpload}
        Good_sim_reg_status_Item_Entries=`expr ${Good_sim_reg_status_Item_Entries} + 1`
      fi
    ...
    Allsim_reg_status_ItemCategories=""; Allsim_reg_status_ItemCategories=`cat ${File} | awk -F'","' '{print $57}' | tr -d '"' | tr ' '  '_' | sort | uniq` # UPDATE_HERE: Make this your field - Example: $57
    for SingleCategory in `echo ${Allsim_reg_status_ItemCategories}`
    do
      Search_String=""; Search_String=`echo ${SingleCategory} | tr '_' ' '`
      Amount=`cat ${File} | awk -F'","' '{print $57}' | grep "\"${Search_String}\"" | wc -l`                           # UPDATE_HERE: Make this your field - Example: $57
      echo " - Field Value: ${SingleCategory} = ${Amount} Entries"
      echo "echo \"LMS.Totals.Device.sim_reg_status_Item.Category.${SingleCategory} ${Amount} ${File_Epoc}\" | nc ${GraphiteServer} ${GraphitePort}" >> ${ScriptDir}/${ScriptNameGraphiteUpload}
    done

    echo "echo \"LMS.Totals.Device.sim_reg_status_Item.Totals ${Good_sim_reg_status_Item_Entries} ${File_Epoc}\" | nc ${GraphiteServer} ${GraphitePort}" >> ${ScriptDir}/${ScriptNameGraphiteUpload}
    echo " - Field Value Total: Good = ${Good_sim_reg_status_Item_Entries} Entries"
    echo "echo \"LMS.Totals.Issues.sim_reg_status_Item.Malformed ${Bad_sim_reg_status_Item_Entries} ${File_Epoc}\" | nc ${GraphiteServer} ${GraphitePort}" >> ${ScriptDir}/${ScriptNameGraphiteUpload}
    echo " - Field Value Total: Bad = ${Bad_sim_reg_status_Item_Entries} Entries"
  ...
  echo " "
  echo "Uploading sim_reg_status_Item Data to Graphite"
