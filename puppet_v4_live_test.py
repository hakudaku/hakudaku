...
def main():
    args = sys.argv[1:]
    usage = ('puppet_v4_live_test.py <id> <patch> <build> <host>\n'
             'Example: puppet_v4_live_test.py 126418 0 103 lsv-1746\n'
             'To clean things/remove test package:\n'
             'Run: puppet_v4_live_test.py <host> --clean')

    if '--clean' in args:
        host = [args[0]]
        rm_infradb4_dir = 'rm -rvf /opt/infradb4'
        rm_test_infradb4_rpm = 'yum remove rfi-infradb-rails4 -y'
        ins_prod_infradb4_rpm = 'yum install rfi-infradb-rails4 -y'
        puppet_up = 'svc -u /service/puppet4'
        cmds = [rm_infradb4_dir, rm_test_infradb4_rpm, ins_prod_infradb4_rpm, puppet_up]
        ssh(host, cmds)

    elif not args or len(args) < 4:
        print usage
        sys.exit(1)
